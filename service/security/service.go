...
func respond(c *gin.Context, response any, ge *define.GeneralError) {
	...
	case QuerySettingResponse:
		value.BasicResponseInfo = info
		c.JSON(http.StatusOK, value)
	case UpdateSettingResponse:
		value.BasicResponseInfo = info
		c.JSON(http.StatusOK, value)
	}
}
...
func HandleQuerySetting(c *gin.Context) {
	...
	if err := c.ShouldBind(&request); err != nil {
		respond(c, QuerySettingResponse{}, define.NewGeneralError(source, err, define.LangKeySafetyRequestBodyInvalid))
		return
	}
	user, ge := loadUser(request.BasicSessionInfo, source)
	if ge != nil {
		respond(c, QuerySettingResponse{}, ge)
		return
	}
	setting, ge := environment.DB.SafetyHandle().QuerySetting(environment.DB.Database(), user.UserUniqueID)
	if ge != nil {
		respond(c, QuerySettingResponse{}, ge)
		return
	}
...
func HandleUpdateSetting(c *gin.Context) {
	...
	if err := c.ShouldBind(&request); err != nil {
		respond(c, UpdateSettingResponse{}, define.NewGeneralError(source, err, define.LangKeySafetyRequestBodyInvalid))
		return
	}
	if request.EmergencyLocationShare == nil && request.SOSConfirmation == nil {
		respond(c, UpdateSettingResponse{}, define.NewGeneralError(source, fmt.Errorf("no setting provided"), define.LangKeySafetySettingInvalid))
		return
	}
	user, ge := loadUser(request.BasicSessionInfo, source)
	if ge != nil {
		respond(c, UpdateSettingResponse{}, ge)
		return
	}
	current, ge := environment.DB.SafetyHandle().QuerySetting(environment.DB.Database(), user.UserUniqueID)
	if ge != nil {
		respond(c, UpdateSettingResponse{}, ge)
		return
	}
	locationShare, confirmation := current.EmergencyLocationShare, current.SOSConfirmation
	if request.EmergencyLocationShare != nil {
		locationShare = *request.EmergencyLocationShare
	}
	if request.SOSConfirmation != nil {
		confirmation = *request.SOSConfirmation
	}
	setting, ge := environment.DB.SafetyHandle().UpdateSetting(environment.DB.Database(), user.UserUniqueID, locationShare, confirmation)
	if ge != nil {
		respond(c, UpdateSettingResponse{}, ge)
		return
	}
